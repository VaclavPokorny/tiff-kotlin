...
public class TiffWriter {
    ...
    private static void writeImageFileDirectories(ByteWriter writer, TIFFImage tiffImage) throws IOException {
        ...
        for (int i = 0; i < tiffImage.getFileDirectories().size(); i++) {
            ...
            writeImageFileDirectory(writer, fileDirectory, isLast);
        }

    }

    private static void writeImageFileDirectory(ByteWriter writer, FileDirectory fileDirectory, boolean isLast) throws IOException {
        ...
        for (FileDirectoryEntry entry : fileDirectory.getEntries()) {
            ...
            long valueBytes = entry.getFieldType().getBytes() * entry.getTypeCount();
            if (valueBytes > 4) {
    ...
    private static CompressionEncoder getEncoder(FileDirectory fileDirectory) {
        ...
        encoder = switch (compression) {
            case TiffConstants.Compression.NO -> new RawCompression();
            case TiffConstants.Compression.CCITT_HUFFMAN -> throw new TiffException("CCITT Huffman compression not supported: " + compression);
            case TiffConstants.Compression.T4 -> throw new TiffException("T4-encoding compression not supported: " + compression);
            case TiffConstants.Compression.T6 -> throw new TiffException("T6-encoding compression not supported: " + compression);
            case TiffConstants.Compression.LZW -> new LZWCompression();
            case TiffConstants.Compression.JPEG_OLD, TiffConstants.Compression.JPEG_NEW -> throw new TiffException("JPEG compression not supported: " + compression);
            case TiffConstants.Compression.DEFLATE, TiffConstants.Compression.PKZIP_DEFLATE -> new DeflateCompression();
            case TiffConstants.Compression.PACKBITS -> new PackbitsCompression();
