...
public class ByteReader {
    ...
    public void setNextByte(long nextByte) {
        if (nextByte >= bytes.length) {
            throw new TiffException("Byte offset out of range. Total Bytes: " + bytes.length + ", Byte offset: " + nextByte);
        }
    ...
    private void verifyRemainingBytes(int offset, int bytesToRead) {
        if (offset + bytesToRead > bytes.length) {
            throw new TiffException("No more remaining bytes to read. Total Bytes: " + bytes.length + ", Byte offset: " + offset + ", Attempted to read: " + bytesToRead);
        }
    }
